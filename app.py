...
import edge_tts
import asyncio
import nest_asyncio  # <-- 1. ADDED to allow asyncio in Streamlit's running loop

nest_asyncio.apply() # <-- 2. ADDED to activate the fix
...
if generate_button:
    if text_to_convert:
        ...
        # --- 3. CHANGED: Sanitize the text to remove problematic characters ---
        clean_text = text_to_convert.replace('*', '').replace('_', '').replace('<', '').replace('>', '')

        with st.spinner(f"🎙️ Generating audio with the voice of {selected_voice_name}..."):
            output_file_path = "final_narration.mp3"
            
            # --- Pass the clean_text to the function ---
            success = asyncio.run(generate_audio(clean_text, VOICE, rate_str, output_file_path))
